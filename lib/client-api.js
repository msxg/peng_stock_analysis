...
stockMonitorRequest.kline = (params = {}) => {
  const search = new URLSearchParams();
  Object.entries(params).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== '') {
      search.set(key, String(value));
  ...
  return request(`/api/v1/stock-monitor/kline?${search.toString()}`);
};
...
export const clientApi = {
  ...
  strategy: {
    bluechipDefaults: () => request('/api/v1/strategy/bluechip/defaults'),
    bluechipPools: () => request('/api/v1/strategy/bluechip/pools'),
    createBluechipPool: (payload = {}) =>
      request('/api/v1/strategy/bluechip/pools', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    updateBluechipPool: (poolId, payload = {}) =>
      request(`/api/v1/strategy/bluechip/pools/${encodeURIComponent(poolId)}`, {
        method: 'PUT',
        body: JSON.stringify(payload || {}),
      }),
    deleteBluechipPool: (poolId) =>
      request(`/api/v1/strategy/bluechip/pools/${encodeURIComponent(poolId)}`, {
        method: 'DELETE',
      }),
    createBluechipPoolSymbol: (poolId, payload = {}) =>
      request(`/api/v1/strategy/bluechip/pools/${encodeURIComponent(poolId)}/symbols`, {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    clearBluechipPoolSymbols: (poolId) =>
      request(`/api/v1/strategy/bluechip/pools/${encodeURIComponent(poolId)}/symbols`, {
        method: 'DELETE',
      }),
    updateBluechipPoolSymbol: (poolId, symbolId, payload = {}) =>
      request(`/api/v1/strategy/bluechip/pools/${encodeURIComponent(poolId)}/symbols/${encodeURIComponent(symbolId)}`, {
        method: 'PUT',
        body: JSON.stringify(payload || {}),
      }),
    deleteBluechipPoolSymbol: (poolId, symbolId) =>
      request(`/api/v1/strategy/bluechip/pools/${encodeURIComponent(poolId)}/symbols/${encodeURIComponent(symbolId)}`, {
        method: 'DELETE',
      }),
    bluechipAnalyze: (payload = {}) =>
      request('/api/v1/strategy/bluechip/analyze', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    bluechipBatchAnalyze: (payload = {}, options = {}) =>
      request('/api-internal/bluechip/batch-analyze', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
        timeoutMs: Number(options?.timeoutMs) > 0 ? Number(options.timeoutMs) : LONG_RUNNING_TIMEOUT_MS,
      }),
    saveBluechipBatchSignals: (payload = {}) =>
      request('/api/v1/strategy/bluechip/results/save', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    bluechipSavedSignals: (params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') {
          search.set(key, String(value));
      ...
      return request(`/api/v1/strategy/bluechip/results${search.toString() ? `?${search.toString()}` : ''}`);
    },
    bluechipSavedBatches: (params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') {
          search.set(key, String(value));
      ...
      return request(`/api/v1/strategy/bluechip/results/batches${search.toString() ? `?${search.toString()}` : ''}`);
    },
  },
  stockBasics: {
  ...
  stocks: {
    ...
    history: (stockCode, params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') search.set(key, String(value));
      });
      return request(`/api/v1/stocks/${encodeURIComponent(stockCode)}/history${search.toString() ? `?${search.toString()}` : ''}`);
    },
  },
  marketMetrics: {
    listRules: (params = {}) => {
      ...
      return request(`/api/v1/market-metrics/rules${search.toString() ? `?${search.toString()}` : ''}`);
    },
    createRule: (payload = {}) =>
      request('/api/v1/market-metrics/rules', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    updateRule: (ruleId, payload = {}) =>
      request(`/api/v1/market-metrics/rules/${encodeURIComponent(ruleId)}`, {
        method: 'PUT',
        body: JSON.stringify(payload || {}),
      }),
    daily: (params = {}) => {
      ...
      return request(`/api/v1/market-metrics/daily${search.toString() ? `?${search.toString()}` : ''}`);
    },
    dailyRange: (params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
      ...
      return request(`/api/v1/market-metrics/daily/range${search.toString() ? `?${search.toString()}` : ''}`);
    },
    compute: (payload = {}) =>
      request('/api/v1/market-metrics/compute', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
  ...
  stockScreening: {
    query: (payload = {}) =>
      request('/api/v1/stock-screening/query', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
        timeoutMs: LONG_RUNNING_TIMEOUT_MS,
      }),
    addToMonitor: (payload = {}) =>
      request('/api/v1/stock-screening/actions/add-to-monitor', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    addToBluechipPool: (payload = {}) =>
      request('/api/v1/stock-screening/actions/add-to-bluechip-pool', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
  },
  system: {
    ...
    marketDataQuery: (params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') search.set(key, String(value));
      });
      return request(`/api/v1/system/market-data/query?${search.toString()}`);
    },
    ...
    syncMarketDataAll: (payload) =>
      request('/api/v1/system/market-data/sync-all', {
        method: 'POST',
        body: JSON.stringify(payload || {}),
      }),
    marketDataJobs: (params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') search.set(key, String(value));
      });
      return request(`/api/v1/system/market-data/jobs${search.toString() ? `?${search.toString()}` : ''}`);
    },
    marketDataQuality: (params = {}) => {
      const search = new URLSearchParams();
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') search.set(key, String(value));
      });
      return request(`/api/v1/system/market-data/quality${search.toString() ? `?${search.toString()}` : ''}`);
    },
  },
};
